...
static NvMmuEntry *get_or_alloc_table(NvVmm *vmm, NvMmuEntry *parent_entry, NvMmuAperture aperture) {
  ...
  nv_dma_clflush_range((void *)virt, NV_VMM_PAGE_SIZE);
  ...
  *parent_entry = turing_mmu_encode_pde(phys, aperture);
  nv_dma_clflush_range(parent_entry, sizeof(NvMmuEntry));
  nv_dma_wmb();
...
int nv_vmm_create(NvVmm *vmm, uint64_t va_start, uint64_t va_limit, struct NvDevice* dev) {
  if (!vmm) {
    return -1;
  }
