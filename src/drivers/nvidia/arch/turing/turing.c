...
static int turing_rm_objects_init(NvDevice *dev, NvGspContext *gsp, const GspStaticConfigInfo *static_info) {
  ...
  memset(&subdev_params, 0, sizeof(subdev_params));
  subdev_params.subDeviceId = 0;

  res = nv_rm_alloc(dev, gsp, dev->rm_handles.client, dev->rm_handles.device, NV_RM_HANDLE_SUBDEVICE, NV20_SUBDEVICE_0, &subdev_params, sizeof(subdev_params));
  if (res != 0) {
    kprintf("[NV/RM] Error: Failed to allocate SubDevice object (%d)\n", res);
    return res;
  }
  dev->rm_handles.subdevice = NV_RM_HANDLE_SUBDEVICE;
  kprintf("[NV/RM] SubDevice allocated (Handle: 0x%08x)\n", dev->rm_handles.subdevice);
...
static int turing_init(NvDevice *dev) {
  ...
  if (nv_vmm_create(&dev->vmm, NV_VMM_DEFAULT_VA_START, NV_VMM_DEFAULT_VA_LIMIT, dev) != 0) {
